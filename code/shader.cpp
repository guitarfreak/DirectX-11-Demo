...
struct BloomShaderVars {
	Mat4 viewProj;

	// Modes: 0-Filter, 1-HBlur, 2-VBlur, 3-Add.
	int mode;
	Vec3 _pad;
};

char* d3dBloomShader = HLSL (
	...
	float4 pixelShader(PSInput input) : SV_Target {
		...
		if(vars.mode == 0) {
			...
			float3 result = tex.Sample(samp, input.uv).rgb * weight[0]; // current fragment's contribution

			bool horizontalBlur = vars.mode == 1;

			if(horizontalBlur) {
				for(int i = 1; i < 5; ++i) {
					result += tex.Sample(samp, input.uv + float2(tex_offset.x * i, 0.0)).rgb * weight[i];
					result += tex.Sample(samp, input.uv - float2(tex_offset.x * i, 0.0)).rgb * weight[i];
				}

			} else {
				for(int i = 1; i < 5; ++i) {
					result += tex.Sample(samp, input.uv + float2(0.0, tex_offset.y * i)).rgb * weight[i];
					result += tex.Sample(samp, input.uv - float2(0.0, tex_offset.y * i)).rgb * weight[i];
				}
			}
...
char* d3dParticleShader = HLSL (
	...
	float linearizeDepth(float z) {
		...
		*/
		...
		return -(vars.near * z) / (vars.far*z - vars.far - vars.near*z);
	}
	...
	float4 pixelShader(PSInput input) : SV_Target {
		...
		float alphaScale;
		{
			...
			float zParticle = input.zPos / vars.far;
			float diff = zScene - zParticle;

			float c = saturate(diff * vars.alphaMod);
			c = contrast(c, vars.alphaContrastMod);
			// float c = smoothstep(0,0.02f,diff);

			float c2 = saturate(zParticle * vars.alphaMod2);
			c2 = contrast(c2, 2);
