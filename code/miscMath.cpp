...
struct TimerInterval {
	...
	bool update(double dt, double time, float intervalSeconds = 1) {
		totalTime += dt;
		count++;
		stamp += time;
...
void combinePolygon(Vec2* p0, int count0, Vec2* p1, int count1) {

// 	Vec2* points = getTArray(Vec2, 200);
// 	int pointCount = 0;
// 	Vec2i* edges = getTArray(Vec2i, 200);
// 	int edgeCount = 0;

// 	for(int i = 0; i < count0; i++) {
// 		Line2 a = {points[i], points[i+1 % count0]};

// 		Vec2* insects[2];

// 		for(int j = 0; j < count1; j++) {
// 			Line2 b = {points[j], points[j+1 % count1]};
...
// 	// for(int i = 0; i < count0; i++) points[pointCount++] = p0[i];
// 	// for(int i = 0; i < count1; i++) points[pointCount++] = p1[i];
// 	// for(int i = 0; i < count0; i++) {
// 	// 	edges[edgeCount++] = { i, (i+1)%count0 };
// 	// }
// 	// for(int i = 0; i < count1; i++) {
// 	// 	edges[edgeCount++] = { i + count0, (i+1)%count1 + count0 };
// 	// }

// 	// for(int i = 0; i < edgeCount; i++) {
// 	// 	Vec2 a = points[edges[i].x];
// 	// 	Vec2 b = points[edges[i].y];

// 	// 	Line2 line = {a,b};
// 	// 	bool inside = false;
// 	// 	for(int i = 0; i < 2; i++) {
// 	// 		Vec2* ps = i == 0 ? p0 : p1;
// 	// 		int count = i == 0 ? count0 : count1;
...
Vec2i* arrayDivideRanges(int size, int count) {
	...
	int index = 0;
	int switchIndex;
	int s = evenDivide(size, count, &switchIndex);

	for(int i = 0; i < count; i++) {
		int c = i < switchIndex ? s : s-1;
		ranges[i] = {index, c};
		index += c;
	}
