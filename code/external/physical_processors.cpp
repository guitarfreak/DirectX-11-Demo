...
static inline void _cpuid(int output[4], int leaf, int subleaf = 0) {
...
int physicalProcessors(int * logical_processors) {
    ...
    int systemProcessors = sysinfo.dwNumberOfProcessors;


    int abcd[4] = { 0,0,0,0 };                   // CPUID output
    _cpuid(abcd, 0);                              // CPUID function 0
    ...
    if (vendor == 1) {
        ...
        //////////////////
    	  ...
    	  // Hyper is actually a type so we can't use the name "hyper".
        int _hyper = 0;                           // hyperthreading status: 0 = unknown, 1 = disabled, 2 = enabled
        if (maxLeaf >= 0xB) {                    // leaf 0xB or 0x1F: Extended Topology Enumeration
            int num = 0xB;
            // if (maxLeaf >= 0x1F) num = 0x1F;

            for (int c = 0; c < 5; c++) {
                _cpuid(abcd, num, c);             // enumeration level c
            ...
            _cpuid(abcd, 0xA, 0);                 // performance monitor counters information
            ...
            if (perfVersion == 3 || perfVersion == 4) {
                if (perfNum == 4) {
                    _hyper = 2;                   // 4 performance registers when hyperthreading enabled
                }
        ...
        else if (logicalProc > systemProcessors && systemProcessors > 0 && _hyper == 0) {
        ...
        _cpuid(abcd, 0x80000000);                 // AMD specific CPUID functions
        int maxLeaf8 = abcd[0] & 0xFFFF;         // maximum eax 0x8000.... input for CPUID

        if (maxLeaf8 >= 8) {
            _cpuid(abcd, 0x80000008);
            logicalProc = (abcd[2] & 0xFF) + 1;

            if (maxLeaf8 >= 0x1E) {
                _cpuid(abcd, 0x8000001E);
